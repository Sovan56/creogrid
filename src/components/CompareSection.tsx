...
import { XCircle, CheckCircle2, ShieldX, TrendingDown, RefreshCcw, DollarSign } from 'lucide-react';
...
export default function CompareSection() {
  ...
  const flowWay = [
    ...
    { text: 'Automated outreach, brief tracking, and contracts in one flow', icon: CheckCircle2 },
  ...
  return (
    <section
      id="features"
      className="relative py-24 px-6 overflow-hidden bg-brand-bg/80 border-t border-indigo-500/5"
    >
      {/* Background soft lighting details */}
      <div className="absolute top-[30%] left-[5%] w-[400px] h-[400px] bg-red-500/5 rounded-full blur-[110px] -z-10" />
      <div className="absolute bottom-[20%] right-[5%] w-[350px] h-[350px] bg-brand-success/5 rounded-full blur-[110px] -z-10" />
      ...
      <div className="max-w-5xl mx-auto w-full text-center">
        ...
        <div className="space-y-4 mb-16">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-display font-medium text-white tracking-tight">
            Sound familiar? Let's check the <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-indigo-300">numbers</span>.
          </h2>
          <p className="text-gray-400 max-w-xl mx-auto text-sm md:text-base">
            Enterprise influencer platforms lock small-to-mid brands into expensive contracts before they even hire their first creator. We rebuilt the flow for simplicity.
          </p>
        </div>

        {/* Dynamic Dual Grid Card Panel */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch pt-4">
          
          {/* Section 1: The Old Enterprise Way */}
          <motion.div
            whileHover={{ y: -4 }}
            className="p-6 md:p-8 rounded-2xl bg-red-950/10 border border-red-500/20 text-left relative overflow-hidden flex flex-col justify-between"
          >
          ...
          {/* Section 2: Pure InfluenceFlow */}
          <motion.div
            whileHover={{ y: -4 }}
            className="p-6 md:p-8 rounded-2xl bg-indigo-950/20 border border-brand-primary/30 text-left relative overflow-hidden flex flex-col justify-between shadow-[0_0_30px_rgba(99,102,241,0.15)]"
          >
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  <span className="text-xs font-mono font-bold tracking-widest text-brand-accent uppercase">
                    THE INFLUENCEFLOW STANDARD
                  </span>
                  <h3 className="text-xl md:text-2xl font-display font-semibold text-white">
                    A Clean, Transparent Flow
                  </h3>
                </div>
                <div className="px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/25 text-xs font-bold text-brand-success font-mono">
                  ✓ DEMOCRATIC
                </div>
              </div>
              ...
              <ul className="space-y-4 font-normal">
                {flowWay.map((item, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <item.icon className="w-5 h-5 text-brand-success shrink-0 mt-0.5" />
            ...
            <div className="pt-8 border-t border-brand-primary/10 mt-8 flex flex-col gap-1">
              <span className="text-xs font-mono text-indigo-400">Basic search is free, Pro scale is</span>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold font-mono text-white">$49/month</span>
                <span className="text-xs text-brand-success font-bold font-mono">(Saves $23.4k/yr!)</span>
              </div>
            </div>
          </motion.div>
